...
Table~\ref{tab:repair_ablation} reports midpoint test accuracy at each REPAIR epoch for both pair types. Two epochs (the default used throughout this work) already recover the majority of the gap; accuracy continues to improve without a clear plateau up to five epochs, suggesting our two-epoch result is a conservative lower bound on the pipeline's potential. We nevertheless retain two epochs in the main results, for constant per-pair cost.
